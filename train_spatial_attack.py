...
from models.resnet_cifar10 import ResNet18 as Resnet18CIFAR10
...
def args_parser():
    ...
    parser.add_argument('--model_dir', type=str, default="./ablation/")
...
def test(model, test_loader, vars):
    ...
    for batch_idx, (data, label) in enumerate(test_loader):
        data, label = spatial_poison(args, data, label, args.target_label, poisoning_frac=1.0, vars=vars)
        data = data.to(args.device)
        label = label.to(args.device)
        output = model(data)
...
def main(selected_vars=None):
    ...
    if args.data == "imagenet_full":
        ...
        if args.model == "vgg16":
            ...
            if args.data == "stl10":
                ...
                model = Resnet18CIFAR10(num_classes=args.num_classes)
    ...
    if selected_vars is None:
        vars_save_file_name = str(args.n_objs) + 'objs_' + args.surrogate_model + '_' + args.data + '_selected_spatialTrigger.pt'
        vars = torch.load('./ablation/trigger_component_vars/' + vars_save_file_name)
        print('loaded vars:' + vars_save_file_name)
        vars = vars[0]
    else:
        print('receive trigger from parameter of main function')
        vars = selected_vars
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, label) in enumerate(train_loader):
            ...
            data, label = spatial_poison(args, data, label, args.target_label, poisoning_frac=args.poison_ratio, vars=vars)
            data = data.to(args.device)
            label = label.to(args.device)
            output = model(data)
            batch_acc = (output.argmax(1) == label.view(-1, )).float().sum()
    ...
    save_file_name = str(args.n_objs) + 'objs_' + args.model + '_' + args.dataset + 'l2=' + '%.2f'%info_dict['l2norm'] + '.pt'
    torch.save(info_dict, './ablation/' + save_file_name)
    print('')
...
if __name__ == '__main__':
    ...
    a = torch.load('./ablation/trigger_component_vars/3objs_vgg11_cifar10_spatialTrigger_L.pt')
